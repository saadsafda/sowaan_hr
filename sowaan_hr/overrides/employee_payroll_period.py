...
class EmployeePayrollPeriod(PayrollPeriod):
	def validate(self):
...
def get_period_factor(
	...
	# frappe.msgprint(str(payroll_period.name))
	...
	if payroll_frequency == "Monthly" and not depends_on_payment_days:
		...
		total_sub_periods = int( flt(days_in_payroll_period) / 30 )
		remaining_days_in_payroll_period = date_diff(period_end, start_date) + 3
		remaining_sub_periods = int( flt(remaining_days_in_payroll_period) / 30 )
