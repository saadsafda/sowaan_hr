...
class TestKSAGratuity(FrappeTestCase):
	def setUp(self):
		frappe.db.delete("KSA Gratuity")
		frappe.db.delete("Salary Slip")
		frappe.db.delete("Additional Salary", {"ref_doctype": "KSA Gratuity"})
	...
	def test_gratuity_based_on_current_slab_via_additional_salary(self):
		...
		salary_slip = make_salary_slip("Test KSA Gratuity", employee=employee)
	...
	def test_gratuity_based_on_all_previous_slabs_via_payment_entry(self):
		...
		pe = get_payment_entry_for_employee("KSA Gratuity", gratuity.name)
...
def create_gratuity(**args):
	...
	gratuity = frappe.new_doc("KSA Gratuity")
...
def create_salary_slip(employee):
	if not frappe.db.exists("Salary Slip", {"employee": employee}):
		...
		salary_slip = make_employee_salary_slip(
			employee, "Monthly", "Test KSA Gratuity", posting_date=posting_date
		)
...
# class TestKSAGratuity(unittest.TestCase):
# 	@classmethod
# 	def setUpClass(cls):
...
# 	def setUp(self):
# 		frappe.db.sql("DELETE FROM `tabKSAGratuity`")
# 		frappe.db.sql("DELETE FROM `tabAdditional Salary` WHERE ref_doctype = 'KSA Gratuity'")
...
# 		pay_entry = get_payment_entry("KSA Gratuity", gratuity.name)
