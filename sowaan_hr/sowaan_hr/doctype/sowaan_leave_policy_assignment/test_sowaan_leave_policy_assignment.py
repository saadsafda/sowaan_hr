...
from sowaan_hr.sowaan_hr.doctype.sowaan_leave_policy_assignment.sowaan_leave_policy_assignment import (
...
class TestSowaanLeavePolicyAssignment(FrappeTestCase):
	def setUp(self):
		for doctype in [
			"Leave Period",
			"Leave Application",
	...
	def test_grant_leaves(self):
		...
		self.assertEqual(
			frappe.db.get_value("Sowaan Leave Policy Assignment", leave_policy_assignments[0], "leaves_allocated"),
			1,
		)
		...
		leave_allocation = frappe.get_list(
			...
			filters={
				...
				"leave_policy": leave_policy.name,
				"sowaan_leave_policy_assignment": leave_policy_assignments[0],
				"docstatus": 1,
			},
		)[0]
		...
		self.assertEqual(leave_alloc_doc.sowaan_leave_policy_assignment, leave_policy_assignments[0])
	...
	def test_earned_leave_alloc_for_passed_months_based_on_joining_date(self):
		...
		effective_from = frappe.db.get_value(
			"Sowaan Leave Policy Assignment", leave_policy_assignments[0], "effective_from"
		)
