...
class SowaanLeavePolicyAssignment(Document):
	def validate(self):
		self.set_dates()
	...
	def validate_policy_assignment_overlap(self):
		leave_policy_assignments = frappe.get_all(
			"Sowaan Leave Policy Assignment",
			filters={
	...
	def create_leave_allocation(
		...
		allocation = frappe.get_doc(
			dict(
				...
				sowaan_leave_policy_assignment=self.name,
	...
	def get_leaves_for_passed_months(
		...
		from sowaan_hr.sowaan_hr.utils import get_monthly_earned_leave

		leave_setting = frappe.get_all("Leave Type Settings", filters={
                                           "leave_type": leave_type_details.get(leave_type).name},
                                           fields=["*"]
                                           )
		...
		if len(leave_setting) > 0 and leave_setting[0].leave_frequency == "Daily":
			...
			if days_passed > 0:
				daily_earned_leave = get_monthly_earned_leave(
					new_leaves_allocated,
					leave_setting[0].leave_frequency,
					leave_type_details.get(leave_type).rounding,
				)
				print('daily_earned_leave***')
				print(daily_earned_leave, days_passed, leave_setting[0].leave_frequency)
				new_leaves_allocated = daily_earned_leave * days_passed
			else:
				new_leaves_allocated = 0
		else:
			...
			if months_passed > 0:
				...
				print('monthly_earned_leave***')
				print(monthly_earned_leave, days_passed)
...
def create_assignment_for_multiple_employees(employees, data):
	...
	for employee in employees:
		assignment = frappe.new_doc("Sowaan Leave Policy Assignment")
