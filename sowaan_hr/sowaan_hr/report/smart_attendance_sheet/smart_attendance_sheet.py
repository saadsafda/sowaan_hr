...
status_map = {
	...
	"On Leave": "L",
	"Present": "P",
	"Late": "<span style='color:red;'>PL</span>",
	"Work From Home": "WFH"
	}
...
def execute(filters=None):
	...
	if filters.summarized_view:
		...
		columns.extend([_("Required Hours") + ":Float:120", _("Provided Hours") + ":Float:120", _("Less Hours") + ":Float:120"])
	...
	if filters.group_by:
		...
		for parameter in group_by_parameters:
			...
			if (att_map_set & emp_map_set):
				delta = getdate(filters.to_date) - getdate(filters.from_date)
				parameter_row = ["<b>"+ parameter + "</b>"] + ['' for day in range(delta.days+2)]
	...
	for idx, day in enumerate(days, start=0):
		...
		for emp in emp_att_map.keys():
			if emp_att_map[emp][idx]:
				...
				if emp_att_map[emp][idx] == status_map.get("Late", ""):
					total_lates_on_day += 1
		...
		datasets[3]["values"].append(total_lates_on_day)
...
def add_data(employee_map, att_map, filters, holiday_map, conditions, default_holiday_list, leave_types=None):
	...
	for emp in employee_map:
		...
		for day in range(delta.days+1):
			date = (getdate(filters.from_date) +  datetime.timedelta(days=day)).strftime("%Y-%m-%d")
			status = None
			status = att_map.get(emp).get(getdate(date).day)
			...
			if status is None and holiday_map:
				...
				if emp_holiday_list in holiday_map:
					for idx, ele in enumerate(holiday_map[emp_holiday_list]):
						if getdate(date).day == holiday_map[emp_holiday_list][idx][0]:
			...
			if  filters.summarized_view:
				if status == "Present" or status == "Work From Home" or status == "Late":
					total_p += 1
				elif status == "Absent":
					total_a += 1
...
def get_columns(filters):
	...
	# print(getdate(filters.to_date) - getdate(filters.from_date));
	days = []
	delta = getdate(filters.to_date) - getdate(filters.from_date)
	for day in range(delta.days+1):
		date = (getdate(filters.from_date) +  datetime.timedelta(days=day)).strftime("%Y-%m-%d")
		day_name = day_abbr[getdate(date).weekday()]
		days.append(cstr(getdate(date).day)+ " " +day_name +"::65")
	if not filters.summarized_view:
		columns += days
...
def get_attendance_list(conditions, filters):
	attendance_list = frappe.db.sql("""select employee, day(attendance_date) as day_of_month,
		status, late_entry from tabAttendance where docstatus = 1 %s order by employee, attendance_date""" %
	...
	for d in attendance_list:
		...
		att_map[d.employee][d.day_of_month] = 'Late' if d.late_entry else d.status
	...
	for d in holiday_list:
		if d:
			holiday_map.setdefault(d, frappe.db.sql('''select day(holiday_date), weekly_off from `tabHoliday`
				where parent=%s and holiday_date between %s and %s''', (d, filters["from_date"], filters["to_date"])))
